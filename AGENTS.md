...
## Метод работы: этап → сборка → проверка владельцем

Работа ведётся этапами. Этап — это связная группа коммитов с одной целью.

Порядок для **каждого** этапа:

1. Сделать коммиты этапа (по одному на изменение, не сваливать в один).
2. Прогнать `scripts/verstak-check.sh` — все проверки должны быть зелёными.
3. Собрать локальные пакеты: `scripts/build.sh`, затем `scripts/package-deb.sh`
   и `scripts/package-appimage.sh`; при изменениях в плагинах — `build.sh` в
   `verstak-official-plugins`.
4. Сообщить владельцу абсолютные пути к артефактам и остановиться.
5. Дождаться отмашки. Следующий этап не начинать, «мелочи» в ожидании не
   доделывать.

Владелец проверяет сборку на своём реальном vault. О критичном он сообщает
сразу, остальные замечания копит и отдаёт после проверки.

Несколько этапов подряд без остановки допустимы **только** по явному
разрешению владельца.

### Проверка в настоящем движке

Всё, что зависит от отрисовки, проверяется в реальном приложении до отправки
владельцу: `scripts/gui-probe.sh` запускает сборку на Xvfb в одноразовом
`$HOME` и снимает скриншот, `--keep` оставляет её запущенной для `xdotool`.
Playwright гоняет оболочку в Chromium — это другой движок, и он не видит ни
поведения нативных полос прокрутки, ни того, что глобальное правило кнопок
съело иконку. Две правки полосы прокрутки и пустая кнопка закрытия в окне
настроек прошли мимо всех тестов и нашлись только так.

### Плагины в сборке

`verstak-desktop/plugins/` — это установленная копия, а не исходники. После
правки манифеста или кода в `verstak-official-plugins` нужно пересобрать
пакеты там (`scripts/build.sh`) и переустановить (`scripts/install-dev-plugins.sh`
в verstak-desktop), иначе сборка поедет со старыми манифестами. Порядок
вкладок Дела не применялся именно поэтому.

Приоритеты и продуктовые решения принимает владелец. Задача агента — находить,
объяснять и чинить, а не решать, что важнее для продукта.
