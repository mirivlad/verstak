...
* файлы, заметки и журнал работ остаются читаемыми вне приложения — журнал
  лежит в самом Деле обычным Markdown, по файлу на месяц
  (`<Дело>/Журнал/2026-07.md`);
...
Пассивный учёт активности по страницам по умолчанию отключён. После включения
расширение передаёт ограниченные интервалы времени для открытой страницы — из
адреса убирается всё после `#`. Оно не отправляет заголовки и содержимое
страниц, нажатия клавиш и полную историю посещений, а по исключённым доменам не
передаёт ничего.

Записанное время превращается в записи журнала без ручной сортировки. Страница,
открытая во время работы над Делом, предлагается этому Делу; Журнал
спрашивает, а не утверждает, и ответ становится правилом для этого адреса —
второй раз о нём не спросят. Все правила видны и снимаются, а у каждой строки
указано, откуда взялось Дело: от вас, из правила или из предположения.
